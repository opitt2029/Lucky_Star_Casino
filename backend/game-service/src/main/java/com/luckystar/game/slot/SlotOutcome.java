 ...
 * @param win          中線是否命中（三連或左二同）
 * @param multiplier   命中倍率（三連 tripleMultiplier 或左二同 pairMultiplier；未中為 0）
 * @param payout       派彩金額（= 下注 x 倍率；未中為 0），含本金返還
 * @param winningCells 命中格座標 {@code [row, col]}：三連為中線三格、左二同為中線左二格、未中為空陣列
 */
