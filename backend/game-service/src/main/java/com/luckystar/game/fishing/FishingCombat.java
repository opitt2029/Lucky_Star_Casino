...
public final class FishingCombat {
     ...
     * 銅 14 / 銀 22 / 金 32——傷害差距刻意收斂（金炮約銅炮 2.3×）：金炮速殺但不致把中小魚 HP 壓到「1~2 發秒殺」。
     * 差距若拉太大，中小魚的 {@link #expectedShotsToKill E[N]} 會被壓得極小，而
     * {@code pCapture = TARGET_RTP × E[N] / multiplier} 會跟著掉到很低，玩家頻繁遇到「血歸零卻掙脫」、體感極差。
     * 現行值下 {@code pCapture} 約落在：銅 0.57~0.86、銀 0.37~0.48、金 0.25~0.48（魚種越大越低）。
     * RTP 仍由 {@code pCapture} 補償成各魚種/砲台一致，故砲台只決定火力/節奏/變異度，不決定期望回報。
     * 注額已與砲台解耦（玩家自選，ADR-004）。
     *
     * <p><b>勿對低 pCapture 設硬地板</b>：那會直接打破上面的等式，使 RTP 破表（ADR-004）。
     * 要改善「血歸零卻掙脫」的體感，只能縮小砲台之間的傷害差距。
     */
