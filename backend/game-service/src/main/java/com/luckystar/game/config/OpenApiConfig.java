 ...
 * game-service 無 spring-security，無需放行設定。對外 JWT 由 gateway 驗證，
 * 此處宣告 bearerAuth 僅供文件層級標註與線上測試帶 token。
 */
@Configuration
@OpenAPIDefinition(
    info = @Info(title = "遊戲服務 API", version = "v1",
                 description = "Lucky Star Casino 遊戲服務端點（老虎機、百家樂、捕魚機、RNG 公平性驗證、RTP 統計）"),
