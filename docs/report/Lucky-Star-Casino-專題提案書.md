...
| **分工方式** | 依模組垂直切分：張鈞皓（組長，會員/Gateway/文件）、黃崇瑜（遊戲引擎）、林瑋彧（錢包/鑽石）、許銘仁（排行/後台/推播/DevOps）、王竣揚（前端）；跨模組事項（DB Schema、整合測試）全員共同參與。王竣揚目前因故無法投入，前端工作由張鈞皓、黃崇瑜、林瑋彧暫代 |
...
> 每頁先列灰階線框草稿（規劃版面），其下附目前實作的完成畫面（紅框標註為功能說明）。草稿由王竣揚 繪製、張鈞皓 審閱定稿（見 4-6）。
...
> 原則：依模組垂直切分、每項任務單一負責人（明細見工作分配表 T-000～T-107）；頁面草稿由王竣揚 繪製、張鈞皓 審閱。

### 模組負責總表

| 成員 | 負責模組 | 範圍說明 |
|---|---|---|
| **張鈞皓（組長）** | Member Service、Gateway、全域/收尾文件 | 註冊/登入/JWT、好友、簽到、新手禮；Gateway 路由/驗證/限流/熔斷；架構決策（ADR）、README、結業簡報 |
| **黃崇瑜** | RNG Game Service | Provably Fair RNG、老虎機與百家樂邏輯/API、Redis 遊戲 Session、公平性驗證、RTP 統計 |
| **林瑋彧** | Wallet Service、鑽石系統（API） | 星幣開戶/扣款/派彩/流水、冪等與樂觀鎖、贈幣、破產補助、DLQ；鑽石開戶/序號兌換/鑽石換星幣 |
| **許銘仁** | Rank、Admin、Notification、測試/DevOps | 全服/好友排行榜、排程快照；後台帳號管理/報表/GM 工具；WebSocket 推播；Docker、JMeter 壓測、E2E |
| **王竣揚** | 前端（全部頁面） | 9 個頁面 + Redux 狀態 + WebSocket 連線 + RWD；版面草稿繪製 |
| **全員** | 跨模組 | 資料庫 Schema 設計（T-006）、End-to-End 整合測試（T-093） |

> ⚠️ **人力調整**：王竣揚（前端）目前因故無法投入，其前端工作由張鈞皓（組長）、黃崇瑜、林瑋彧三人暫代分攤。

### 每頁草稿／每項功能 → 負責人

| 頁面（草稿＋實作） | 草稿 | 前端實作 | 對應後端 API | 後端負責人 |
|---|---|---|---|---|
| 首頁 | 王竣揚（張鈞皓 審閱） | 王竣揚 | —（靜態導覽） | — |
| 登入/註冊頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-080） | 註冊/登入/刷新/登出 | 張鈞皓（T-010~T-013） |
| 遊戲大廳 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-082） | 餘額查詢 | 林瑋彧（T-021） |
| 老虎機頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-083） | slot spin/round API | 黃崇瑜（T-030~T-033） |
| 百家樂頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-087） | baccarat bet/result API | 黃崇瑜（T-034~T-036） |
| 鑽石錢包頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-107） | 鑽石開戶/兌換/餘額 | 林瑋彧（T-101~T-104） |
| 排行榜頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-085） | 全服/好友榜查詢 | 許銘仁（T-040~T-042） |
| 會員中心頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-088） | 個資 CRUD/好友/簽到 | 張鈞皓（T-014~T-017） |
| 交易紀錄頁 | 王竣揚（張鈞皓 審閱） | 王竣揚（T-086） | 帳務流水查詢 | 林瑋彧（T-025） |
...
| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-000 | GitHub Repo 與分支策略 | 張鈞皓 | 05/29 | ✅ |
| T-001 | 架構圖定稿與 ADR | 張鈞皓 | 05/29 | ✅ |
| T-002 | Docker Compose 環境 | 許銘仁 | 05/29 | ⚠️ |
| T-003 | 各 Service 專案初始化 | 許銘仁 | 05/29 | ✅ |
| T-004 | React 前端初始化 | 王竣揚 | 05/29 | ✅ |
| T-005 | Kafka Topic 規劃 | 張鈞皓 | 05/29 | ✅ |
...
| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-010~T-015 | 會員註冊/登入/JWT/個資/好友 | 張鈞皓 | 06/03 | ✅ |
| T-020、T-021 | 錢包開戶、餘額查詢 | 林瑋彧 | 06/03 | ✅ |
| T-030、T-031 | RNG 演算法、老虎機邏輯 | 黃崇瑜 | 06/03 | ✅ |
| T-060 | Gateway 路由設定 | 張鈞皓 | 06/03 | ✅ |
| T-080 | 登入/註冊頁面 | 王竣揚 | 06/03 | ✅ |

### S1-W3 核心後半（預定 06/07 完成）

| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-016~T-018 | 任務系統/簽到/新手禮 | 張鈞皓 | 06/07 | ✅ |
| T-022~T-024 | 扣款/派彩/冪等機制 | 林瑋彧 | 06/07 | ✅ |
| T-032 | 老虎機 API | 黃崇瑜 | 06/07 | ✅ |
| T-040 | 全服排行榜（ZSet） | 許銘仁 | 06/07 | ✅ |
| T-061 | Gateway JWT 過濾器 | 張鈞皓 | 06/07 | ✅ |
| T-081 | Redux 全域狀態 | 王竣揚 | 06/07 | ✅ |

### S2-W4 業務整合（預定 06/11 完成）

| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-025~T-027 | 流水查詢/贈幣/破產補助 | 林瑋彧 | 06/11 | ✅ |
| T-033、T-034 | Redis Session、百家樂邏輯 | 黃崇瑜 | 06/11 | ✅ |
| T-041、T-042 | 好友榜、排行查詢 API | 許銘仁 | 06/11 | ✅ |
| T-062、T-063 | 限流、熔斷 | 張鈞皓 | 06/11 | ✅ |
| T-082、T-083 | 大廳、老虎機頁面 | 王竣揚 | 06/11 | ✅ / ⚠️ |
| T-100~T-102、T-104 | 鑽石 Schema/開戶/序號兌換/餘額 | 許銘仁 / 林瑋彧 | 06/11 | ✅ |

### S2-W5 Kafka 串接與擴充（預定 06/18 完成）⬅ 本週

| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-028 | Kafka DLQ 處理 | 林瑋彧 | 06/18 | ✅ |
| T-035~T-037 | 百家樂 API、公平性驗證、RTP 統計 | 黃崇瑜 | 06/18 | ✅ |
| T-043~T-045 | 週榜重置/每日快照/贏幣王 | 許銘仁 | 06/18 | ⬜ |
| T-050~T-055 | Admin 後台六項 API | 許銘仁 | 06/18 | ⬜ |
| T-070~T-073 | Notification 推播四項 | 許銘仁 | 06/18 | ⬜ |
| T-084~T-089 | WebSocket/排行/帳務/百家樂/個資頁、RWD | 王竣揚 | 06/18 | ⚠️（RWD ❓） |
| T-092 | Swagger API 文件 | 許銘仁 | 06/18 | ⬜ |
| T-103 | 鑽石兌換星幣 API | 林瑋彧 | 06/18 | ✅ |
| T-105、T-106 | 點數卡後台 API | 許銘仁 | 06/18 | ⬜ |
| T-107 | 鑽石錢包頁面 | 王竣揚 | 06/18 | ✅ |

### S3 測試週（預定 06/25、06/29 完成）

| 編號 | 任務 | 負責人 | 預定完成日 | 狀態 |
|---|---|---|---|---|
| T-090 | JMeter 高併發壓測 | 許銘仁 | 06/25 | ⚠️ |
| T-091 | 帳務一致性自動驗證 | 許銘仁 | 06/29 | ⬜ |
...
| T-094 | README 與部署文件 | 張鈞皓 | 07/03 | ✅（提前） |
| T-095 | ADR 文件整理 | 張鈞皓 | 07/03 | ⚠️ |
| T-096 | 結業簡報 | 張鈞皓 | 07/03 | ⬜ |
