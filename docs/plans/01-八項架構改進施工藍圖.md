...
| P3 | 捕魚 Redis session 原子化（Lua CAS） | M | ⬜ **未動工**（2026-07-13 複核仍是唯一未開工項；`FishingSessionStore` 尚無 `version`／Lua script。ADR-008 編號已保留） |
...
> **2026-07-13 複核：8 項中 7 項已完成，僅 P3（捕魚 Lua CAS）未動工**——若要接手，下面 Phase 3 節就是完整施工說明。
