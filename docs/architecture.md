...
|---------|---------|------|-----|--------|
| `refresh:{memberId}` | String | Refresh Token（一人一把 → 新登入踢掉舊裝置） | 7 天 | Member Service |
| `jwt:blacklist:{jti}` | String | JWT 黑名單（已登出 Token） | Token 剩餘有效期 | Member Service |
...
| `game:session:{roundId}` | Hash | 遊戲 Session（serverSeed、下注額、狀態） | 30 分鐘 | Game Service |
...
| `rate:player:{playerId}` | 計數器 | 玩家 API 限流 | 滑動視窗 | Gateway |
