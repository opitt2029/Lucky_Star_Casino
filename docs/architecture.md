...
| **Kafka 發布** | `member.registered`（註冊完成）、`friend.relationship.updated`（好友完整清單更新） |
...
|---------|---------|------|-----|--------|
| `refresh:{memberId}` | String | Refresh Token 儲存（`refreshToken()` 每次換發時 rotate + 重設 TTL，非固定倒數） | `jwt.refresh-token-expiry-ms`（現行 7 天） | Member Service |
...
|-------|-----------|-------------|--------|--------|
| `member.registered` | 1 | 1 | Member Service | Wallet Service、Member Service、Rank Service |
| `wallet.debit` | 1 | 1 | Wallet Service | Rank Service |
| `wallet.credit` | 1 | 1 | Wallet Service | Rank Service |
| `friend.relationship.updated` | 3 | 1 | Member Service | Rank Service |
...
| `member.registered.DLT` | 1 | 1 | Kafka（自動） | Admin Service（手動重試） |
| `friend.relationship.updated.DLT` | 1 | 1 | Kafka（自動） | Admin Service（手動重試） |

> **DLT（Dead Letter Topic）**：Consumer 處理失敗超過 3 次的訊息自動轉入，管理員可查詢並手動重試。
