...
export default function Diamond() {
  ...
  const { diamondBalance, exchangeRate, loading, error, lastRedeemAmount, successMessage } =
  ...
  const exchangePreview =
    Number.isInteger(numericExchangeAmount) && numericExchangeAmount > 0
  ...
  const handleRedeem = async (event) => {
    ...
    try {
      ...
      dispatch(
        setDiamondSuccessMessage(`兌換成功，本次獲得 ${redeemedDiamonds.toLocaleString()} 鑽石`)
      )
      setCardCode('')
    } catch (apiError) {
  ...
  const handleExchange = async (event) => {
    ...
    try {
      ...
      const diamondBalanceAfter =
        result.diamondBalanceAfter ?? diamondBalance - numericExchangeAmount

      dispatch(setDiamondBalance(diamondBalanceAfter))
      if (typeof result.starBalanceAfter === 'number') {
        dispatch(
          setBalance({ balance: result.starBalanceAfter, frozenAmount: wallet.frozenAmount })
        )
      } else {
        dispatch(
          setBalance({ balance: wallet.balance + starAmount, frozenAmount: wallet.frozenAmount })
        )
        dispatch(fetchWallet())
      }
    ...
    <AppShell>
              ...
              >
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-3">
                  {[
                    ...
                    [
                      '固定匯率',
                      `鑽石可依 1:${resolvedExchangeRate.toLocaleString()} 的匯率兌換為星幣。`,
                    ],
                ...
                <div className="mt-4 rounded border border-yellow-200/15 bg-red-950/70 p-4">
                  ...
                  <div className="mt-3 grid gap-2 text-sm font-bold leading-6 text-yellow-100/70">
                    <p>
                      鑽石與星幣皆為平台模擬幣，僅供 Lucky Star Casino
                      站內娛樂體驗使用，無現金價值。
                    </p>
                    <p>
                      序號兌換與鑽石換星幣紀錄以系統交易紀錄為準；若發現異常，請先停止操作並聯絡客服。
                    </p>
                    <p>
                      不得使用非本人取得、偽造、外掛或異常來源的序號；平台可依風控規則暫停可疑兌換。
                    </p>
                  </div>
                </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-center">
          ...
          <div className="diamond-hero-gem" aria-hidden="true">
            <div className="diamond-hero-gem__showcase">
              <img
                src="/backgrounds/diamond-wallet-real-gems.png"
                alt=""
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
        <form
          ...
          className="diamond-action-panel luxury-panel-soft rounded p-5 sm:p-6"
        >
