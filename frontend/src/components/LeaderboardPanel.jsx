...
function secondaryText(row) {
  ...
  return id == null ? '公開排行榜資料' : `玩家 ID ${id}`
}
      ...
      {displayRows.length > 0 && (
        <div className="mt-4 space-y-2">
          {displayRows.map((row, index) => {
            const id = row.playerId ?? row.id
            const nickname = row.nickname || row.name || `玩家 ${id ?? index + 1}`
            const displayedRank = Number(row.rank) > 0 ? Number(row.rank) : index + 1
            const isMe = myPlayerId && String(id) === String(myPlayerId)
            return (
              <button
                key={id ?? `${nickname}-${index}`}
                type="button"
                ...
                aria-label={`查看 ${nickname} 的公開排行榜資訊`}
              >
                ...
                <RankPlayerAvatar player={{ ...row, nickname }} size="sm" />
                <span className="min-w-0">
                  <span className={['block truncate font-black', isMe ? 'text-red-950' : 'text-yellow-100'].join(' ')}>{nickname}</span>
