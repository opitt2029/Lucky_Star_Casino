...
14. **前端 mock 玩法必須鏡像後端引擎（單一真相＝後端）**：前端預設走 mock（`gameApi.js`：`VITE_USE_MOCK_API !== 'false'`），所以 `frontend/src/services/mockApi.js` 的玩法/賠付就是預設玩家實際體驗到的。**改後端遊戲規則（權重/倍率/補牌/結算）時，必須同步改 mock**，否則兩個世界分歧。已對齊基準：老虎機（`SLOT_PAYTABLE` ↔ `SlotSymbol`：逐格加權、中線三連、倍率綁符號）、百家樂（`bankerDrawsMock` ↔ `BaccaratGameService.bankerDraws`：補牌表、天牌、**和局押莊/閒 push 退本金**、莊贏扣 5% 傭金）、捕魚（命中率 `0.92/倍率`）。**勿在 mock 加「強制中獎率」或隨機倍率**等後端沒有的機制。
