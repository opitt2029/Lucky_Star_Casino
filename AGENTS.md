...
10. **服務完成度**：member / gateway / wallet 已實作；rank 已完成 T-040~T-042 排行榜核心；**game 已完成 T-030~T-037 全部**（Provably Fair RNG / 老虎機滾輪邏輯 / `POST /api/v1/game/slot/spin` / Redis Session 兩階段 commit-ahead / 百家樂邏輯+API `/bet`+`/result` / RNG 公平性驗證 API / 遊戲 RTP 統計排程+API）；**admin 已完成 T-050 認證地基**（獨立 `ADMIN_JWT_SECRET`、SUPER_ADMIN/OPERATOR 角色、Spring Security `/admin/**`、`POST /admin/auth/login`、`admin_users` 表 + seeder；業務 API T-051~T-055/T-105/T-106 尚未做）；**notification 服務尚未建立**。動工前先看 AUDIT_REPORT 附錄 A 與 CHANGELOG 確認。
