...
13. **前端遊戲（slot/baccarat/fishing 及新遊戲）四鐵則**：每個有下注的遊戲都必須遵守，否則會重現「沒錢狂按 / 視覺鎖脫鉤 / 音效當機 / 全螢幕版面跑掉」四類 bug。
    ...
    - **全螢幕容器一律用 flex「主角吃剩餘空間」，勿用固定列數的 grid**：三次事故（#253 按鍵被蓋、#255 轉輪第三行被裁、#257 百家樂/捕魚版面位移）根因完全相同——**容器的高度分配依賴了會變動的東西**：子元素數量（`display: none` 一收，item 就往前遞補，`1fr` 落到空列上）或內容的 max-content（結算面板一長內容，主舞台那列就被扣掉）。正確寫法：容器 `display: flex; flex-direction: column`，主舞台 `flex: 1 1 auto; min-height: 0`，其餘 `flex: 0 0 auto`；資訊型面板要嘛 `overflow-y: auto` 自己捲、要嘛給固定高度（如 `.baccarat-settlement` 的 `clamp(160px, 26vh, 300px)`），**絕不讓它用 `auto` 撐大所在列**。另注意 `overflow: hidden` 的容器**仍可被程式捲動**（瀏覽器把焦點元素捲進可視範圍），一旦 `scrollHeight > clientHeight` 玩家就會看到畫面位移且沒有捲軸能捲回來——所以全螢幕容器的不變量是 `scrollHeight <= clientHeight`。回歸由 `frontend/e2e/fullscreen-layout.spec.js` 守門（真瀏覽器量測，jsdom 沒有版面引擎、單元測試抓不到這類 bug），**改任何遊戲的全螢幕 CSS 後務必跑 `npx playwright test fullscreen-layout.spec.js`**。
...
15. **改老虎機權重要同步改測試**：`SlotSymbol` 權重一變，`SlotSymbolTest`（總和、`fromWeightedIndex` 累積區間）、`SlotMachineTest.spin_rtpWithinExpectedBand`（RTP/命中率區間）與 `ContractParityTest`（`contracts/slot-paytable.json` 相等性，雷區 14）會紅；改完務必同步 `contracts/slot-paytable.json`、跑 `mvn -pl backend/game-service test` 並更新 Javadoc 的理論 RTP/命中率（單中線**兩階賠付**、含本金倍率：RTP=Σpᵢ³·Tᵢ（三連）+Σpᵢ²(1−pᵢ)·Pᵢ（左二同）、命中率=Σpᵢ³+Σpᵢ²(1−pᵢ)；理論值見 `SlotSymbol` Javadoc，現約 RTP 93.5%/命中率 30.7%）。
16. **捕魚機＝PixiJS canvas 引擎 + 血量/傷害模型（已非 DOM、非「每發獨立命中」）**：決策見 `docs/adr/ADR-003.md`、`docs/adr/ADR-004.md`（經濟再平衡）。
    ...
    - **殘血回收要「整場加總後只 floor 一次」**：`FishingCombat.recoveryPayout()` 內含 `Math.floor`，**每呼叫一次就丟掉不到 1 星幣的小數**。曾經是逐發子彈呼叫一次，單發 10 星幣（`MIN_BET`）時有效回收率被壓到 0.62、而非設計值 `RECOVERY_RATE`=0.70，注額越小虧越多（bet≥100 才回到 0.696）。現在 `FishingService.computeResidualRecovery()` 是「把 `fishDamage` 現存值 ＋ `prunedFishDamage` 全部加總 → 呼叫一次 `recoveryPayout()`」，誤差固定為整場 < 1 星幣。**勿改回逐發/逐條各算一次**，也**勿把 floor 改成 round**（round 會讓回收超過投入成本，破壞「整體 RTP ≤ TARGET_RTP」的莊家安全保證）。另注意 `pruneFishDamage()` 淘汰最舊 entry 前必須把累傷併進 `prunedFishDamage`，否則那些子彈變沉沒成本。回歸由 `FishingServiceCrossBatchTest#residualRecoveryIsFlooredOncePerSession` 守門。
    - **命中框是「逐魚種量測值」，換圖必須重量**：`fishingEngine.js` 的 `FISH_HITBOX` 表（`rx`/`ry`/`ox`/`oy`，相對 sprite 顯示寬高的比例）是依現行 PNG 的 alpha 通道逐像素量出來的（魚貼圖都是 1254×1254 方形，魚身佔比每種不同）。在 `casino-fx/assets/registry.js` 的 `ART_OVERRIDES` **換任何一張魚圖，就要重新量測並更新本表**，否則該魚退回 `FISH_HITBOX_FALLBACK` 的鬆散框、重現「沒打到卻算命中」。命中判定本身是 `_pathIntersectsFish()` 的線段 vs 橢圓解析解（座標除以 (rx, ry) 化為單位圓解一元二次），**勿改回多點取樣近似**（斜射會漏判）。另注意此判定是 **hitscan**：開火當下就對「當時的魚座標」定命中，子彈飛行只是演出；且一發子彈只打**彈道上最近的一個目標**（含擋路怪），不會穿透到後面的魚。
    - **依賴**：前端用 `pixi.js`（`package.json`）；`git pull` 後若 build 報 `Rollup failed to resolve import "pixi.js"`＝忘了 `npm install`。
    - **升級進度（2026-07-21 複核）**：Phase 1~4 皆已落地——HP 條/暴擊/傷害浮字、砲台差異化（`CANNON_DAMAGE`）、自動射擊/鎖定/準心/`perfMode`/分頁隱藏暫停、11 種魚含 `Tier.BOSS` 龍王；**Redis session 原子化（Lua CAS，ADR-008）已完成**，八項架構改進全數收工（見 `docs/plans/01-八項架構改進施工藍圖.md`）。
    - **Session 樂觀鎖（ADR-008）**：`FishingSession.version`（`@Builder.Default = 0L`）比照雷區 16 上方原則，**新增/移動欄位時務必同步 `FishingSessionStore.toHash()/fromHash()` 補 `version` 序列化**（已做，未來若重構這段勿漏）。`FishingSessionStore.save()`（純覆寫，無版本檢查）**只保留給 `FishingService.start()`** 建立全新 session；既有 session 的讀改寫（`shots()`/`topUp()`）一律呼叫 `saveCas(session, expectedVersion)`——CAS 失敗（Lua script 比對 `version` 不符）代表期間有其他請求先寫入，**須整批重讀→重放→重存**（`SESSION_CAS_MAX_RETRIES=3`），**不可沿用舊快照的 session 物件繼續往下算**（那等於繞過 CAS 保護、把丟失更新問題原封不動搬回來）。用盡重試拋 `SessionConflictException`（→ 409，`GlobalExceptionHandler` 已註冊）。`topUp()` 的 wallet `debit()` 只在重試迴圈外呼叫一次（冪等鍵固定，重試的只是「把已扣款金額套進 session」這一步，不重打 wallet）。新增其他捕魚寫入路徑（若有）比照此模式，**勿新增繞過 `saveCas` 直接呼叫 `save()` 改既有 session 的路徑**。
...
23. **wallet 的 `wallet.credit`/`wallet.debit` 事件走 Transactional Outbox，勿改回直接 `kafkaTemplate.send()`（藍圖 04 P2）**：`WalletService.credit/debit` 與 `GiftTransferService.transfer` 把事件寫進 `wallet_outbox`（Postgres 寫庫，與 `wallet_transactions` 同一交易、原子），由 `WalletOutboxPoller`（`@Scheduled` + `@Transactional(postgresTransactionManager)`）撈 PENDING、`send().get(10s)` 確認送達才標 SENT。**新增任何 wallet 事件一律呼叫 `WalletOutboxService.save(topic, key, payload)`**（在 `@Transactional(postgresTransactionManager)` 方法內），**勿在 service 內直接裸發 Kafka**——舊寫法是「commit 後非同步 send()、未 `.get()` 也未掛 callback」，broker 失敗發生在背景執行緒、連 `log.warn` 都不印，事件無聲丟失（下游 MySQL 讀視圖/rank 排行/admin 報表三方同時漂移）。序列化失敗由 `WalletOutboxService` 拋 → 交易回滾（寧可入帳也失敗，不留無聲缺口）。entity 必須落 `postgres.entity` 套件（雷區 5 EMF 掃描）；新增 topic 仍要同步 `tests/infra/kafka.test.js`（雷區 7）。投遞保證為 at-least-once，重送由下游冪等吸收（如 rank 消費端去重，藍圖 04 P1）。**（2026-07-29 起）poller 只標 SENT 不刪列，保留期清理交由 `WalletOutboxPurgeJob`（member 為 `OutboxPurgeJob`）每日 04:00 刪掉 `sentAt` 超過 `retention-days`（預設 7 天，與 rank 去重標記 TTL 對齊）的 SENT 列**——**只刪 SENT，PENDING 無論多舊都不可刪**（刪掉＝無聲丟失事件，正是 Outbox 要防的事；PENDING 長期堆積該由 `wallet_outbox_pending_events` 指標告警＋人工處理，不是靠清理掩蓋）。改保留期別短於 7 天：outbox 是「事件到底有沒有發出去」唯一的排查證據，短於去重 TTL 會出現「事件已刪、去重標記還在」的無法對照狀態。
24. **消費端「非冪等累加」必須去重，冪等寫入不可去重（藍圖 04 P1）**：Kafka 是 at-least-once，同一則事件會重投遞。判準是**操作本身冪等嗎**——冪等寫入（如 rank `updatePlayerCoins` 的 `ZADD` 絕對值 `balanceAfter`）要的是「可安全重放」，**加去重反而有害**（首次寫入後、`ack` 前崩潰，重送被擋掉會把值永久卡在錯的中間態）；非冪等累加（如 rank `addDailyWinnings` 的 `ZINCRBY`）才需「只執行一次」，用事件 `transactionId` 做 Redis `SETNX`（key 帶用途前綴如 `rank:dedup:daily-win:{txId}`、TTL > 最大重送窗口）去重。**一個 listener 裡一支冪等一支不冪等的不對稱最容易漏**（見 `WalletBalanceChangedConsumer`）。`transactionId` 為 null 要跳過去重直接執行（勿用 null 組 key）。此為 best-effort（非 exactly-once），若日後接入實際獎勵發放須升級為單一 Lua script 原子版（比照 ADR-008 CAS）。
25. **自訂 Micrometer 指標不可讓 gauge 回呼直接查 DB（藍圖 04 P5）**：Micrometer 的 gauge 回呼是在**每次 Prometheus scrape 時同步執行**（本專案 `scrape_interval: 5s`），回呼裡下 SQL＝每 5 秒打一次資料庫、且抓取執行緒被 DB 延遲拖住。範本＝`wallet-service` 的 `observability/WalletOutboxMetrics`：`@Scheduled` 每 15 秒把 `countByStatus(PENDING)` 寫進 `AtomicLong`，gauge 只讀記憶體；查詢失敗**保留舊值不歸零**（歸零＝假裝積壓已解除，比沒數字更危險）。**Kafka consumer lag 不需自己寫 code**——七個服務都沒有自訂 `ConsumerFactory`/`ProducerFactory` bean，Spring Boot `KafkaMetricsAutoConfiguration` 會自動綁 `kafka_consumer_fetch_manager_records_lag_max`；**若日後有人自訂 `ConsumerFactory` bean，auto-config 會退讓、lag 指標會靜默消失**，屆時必須手動 `addListener(new MicrometerConsumerListener<>(meterRegistry))`。新增指標記得同步 `observability/grafana/provisioning/dashboards/lucky-star-overview.json` 加 panel（gauge 帶 `baseUnit` 時 Prometheus 名稱會**多一個單位後綴**，如 `wallet.outbox.pending` + `events` → `wallet_outbox_pending_events`，PromQL 寫錯名字會是空面板）。
26. **久沒重建的本機 DB volume 會缺 migration，服務開機即死**：`database/*/init.sql` **只在 volume 全新時**由官方 image 的 entrypoint 執行；既有 volume 一律不跑。所以 `git pull` 拿到新 schema（如藍圖 04 P2 的 `wallet_outbox`）後，舊 volume 沒有那張表，wallet-service 會 `Schema-validation: missing table [wallet_outbox]` 無限重啟，並讓 `depends_on: service_healthy` 的 game/gateway 永遠停在 `Created`。**判定**：`docker logs <service> | grep "missing table"`。**解法**：跑 `node tools/db/apply-migrations.mjs`（ledger 表 `schema_migrations` 記錄套過哪些檔、逐語句套用、「已存在」類錯誤自動跳過、可重複執行；`--dry-run` 先看計畫），別急著 `down -v` 砍資料。**注意該工具在 ledger 空白時會從 V1 重播一輪**，其中 `chk_wt_sub_type` 的「DROP 舊的→ADD 較窄清單」會被現有較新資料違反而跳過，靠「重播必定跑到最後一支、最終約束最寬」自癒——所以**新增 migration 不可縮窄既有 CHECK 清單**，否則這個前提破裂（細節見腳本檔頭「約束重播」）。同源問題還有 **`AdminUserSeeder` 只在帳號不存在時播種**——舊 volume 裡的 superadmin 密碼是「當初」的值，改了 `.env` 的 `ADMIN_SEED_PASSWORD` 也不會生效（GM 發幣 401、壓測 provisioning 直接中止）；解法＝`DELETE FROM admin_users WHERE username='superadmin'` 再重啟 admin-service（`admin_users` 無 FK 指向、`admin_action_logs` 不會連帶損失）。另：容器與 network 脫鉤時（`docker inspect <c> --format '{{json .NetworkSettings.Networks}}'` 回 `{}`）會報 `UnknownHostException: postgres`，**必須整組 `docker compose down` 再 `up`**，只 restart 單一容器無效。
27. **`tools/observability/*.ps1` 必須存成 UTF-8 **with BOM****：Windows PowerShell 5.1 把無 BOM 的 UTF-8 當 ANSI 解讀，中文註解變亂碼後會直接觸發語法錯誤（`Missing expression after ','`、`Missing closing '}'`），且錯誤訊息完全指不到真因。用編輯器寫完務必確認 BOM（`[System.IO.File]::WriteAllText($f, $text, [System.Text.UTF8Encoding]::new($true))`）。另外**陣列參數不能經 `powershell -File` 傳**（`-Steps 25,50,100` 會被當成單一字串 → `Cannot convert value ... to type System.Int32[]`），要用 `& script.ps1 -Steps @(25,50,100)` 直接呼叫。
28. **第三方登入（OAuth/OIDC，ADR-011）有四條非顯而易見的約束**：實作在 member-service（`SocialAuthService` / `SocialOAuthSuccessHandler`），不是獨立服務。
    - **member-service 的 session policy 已從 `STATELESS` 改成 `IF_REQUIRED`**（`SecurityConfig`），因為 OAuth authorization-code 握手必須用 HttpSession 存 `state` 與 `socialOAuthMemberId`（綁定流程的會員身分）。**session 存在 JVM 記憶體**，所以：① member-service **只能單副本**——authorize 與 callback 打到不同實例，握手必失敗（目前 gateway 用固定 `MEMBER_SERVICE_URL`，尚未踩到）；② 服務重啟會讓所有進行中的登入／綁定流程失效。要開多副本前必須先接 Spring Session + Redis（專案已有 Redis）。**勿把 policy 改回 `STATELESS`**（OAuth 會整條壞掉），也勿假設 member-service 是無狀態的。
...
31. **Gateway 有「兩套獨立限流」＋熔斷，壓測看到 429/503 先分清是哪一層，調參有固定順序**（`gateway-service/application.yml`）：
    - **`rate-limit.player`＝每玩家 token bucket**（`PlayerRateLimitGlobalFilter`，一般 10/s、game 5/s），懲罰的是**單一玩家**刷太快。**（2026-07-30 起）此 filter 才真的是令牌桶**：舊版是固定視窗 INCR 計數器（視窗邊界可送 2× 上限）＋`replenishRate` 是死設定（只讀 `burstCapacity`，「game 5/s」根本不存在、實際固定每秒 10）＋INCR/EXPIRE 兩次往返（expire 失敗→key 永無 TTL→該玩家永久 429 不自癒）。現改為單一 Lua 令牌桶（改寫自 SCG 內建 `request_rate_limiter.lua`），TTL 與 token 同腳本原子寫入，`replenishRate`/`burstCapacity` 皆生效；key 用 hash tag（`rate:game:{<uid>}:tokens`/`:ts`）。**改此 filter 前先看令牌桶 Lua，勿改回讀改寫或把 TTL 拆成第二次往返**。
    - **`ipKeyResolver`（auth 路由 IP 限流）只在可信代理身後才採信 `X-Forwarded-For`**（2026-07-30 起）：舊版無條件信任 XFF 第一段，攻擊者每請求帶隨機 XFF 就換到新桶、`/api/v1/auth/**` 暴力破解防護等同失效。現以 `rate-limit.trusted-proxies`（CIDR 清單，`TrustedProxyMatcher`）為信任邊界：**空清單（預設）＝完全不信任 XFF、一律用 socket 對端**（gateway 直接暴露 8080 的安全預設）；採信時 hop **由右到左**找第一個非可信 hop。部署在 nginx/LB 後才把其 CIDR 填進 `RATE_LIMIT_TRUSTED_PROXIES`。**勿把 XFF 改回無條件採信，也勿新增 `X-Real-IP`/`Forwarded`/`True-Client-IP` 支援**（都是用戶端可控）。
...
32. **Redis 不只是快取，有兩類資料禁不起清空；`down -v` 會連排行榜一起砍（2026-07-29 起已加 AOF + volume）**：本專案只有 `risk:rtp:{gameType}` 是純快取，其餘都是 Redis 當**主儲存**或跨服務共用狀態。
