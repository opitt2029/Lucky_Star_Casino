...
31. **Gateway 有「兩套獨立限流」＋熔斷，壓測看到 429/503 先分清是哪一層，調參有固定順序**（`gateway-service/application.yml`）：
    - **`rate-limit.player`＝每玩家固定視窗計數器**（`PlayerRateLimitGlobalFilter`，一般 10/s、game 5/s），懲罰的是**單一玩家**刷太快。實作是 Redis `INCR` + 首次請求設 1 秒 TTL（key `rate:player:{id}` / `rate:game:{id}`），視窗到期靠 TTL 自然歸零、無清理排程。**不是 token bucket**（前端 `useFishingSession` 那個才是，有 tokens 與 refill 速率）——差別在邊界：固定視窗允許「視窗尾＋下個視窗頭」瞬間吃到 2 倍配額，壓測若看到略高於設定值的瞬時通過量屬預期、非 bug。Redis 故障時 **fail-open 放行**（限流元件不該自己造成全站中斷；對比 `JwtAuthenticationGlobalFilter` 是 fail-closed，因為放行等於已撤銷的 token 復活）。
